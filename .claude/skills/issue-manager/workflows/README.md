...
- 包含完整历史上下文
- 可以查看代码、执行命令
- **智能触发修复**：当用户表达修复意图时，AI 会自动检查标签并添加 `fix-confirmed` 标签触发修复

**使用方式**:
```bash
# 直接在 issue 中评论，AI 会自动回复
# 如果评论"修"、"同意修复"等，AI 会智能触发修复流程
```
...
**触发时机**:
- Issue 评论中包含明确确认指令时（`/confirm-fix`、`/approve`）
...
**说明**:
- 这是快捷通道，用于明确的确认指令
- 对于自然语言（"修"、"同意修复"等），由 `issue-chat` 工作流智能处理
...
4. **确认或拒绝方案**
   - ✅ **确认**：
     - **方式 1**（推荐）：直接评论自然语言，如"修"、"同意修复"、"开始修复"，AI 会自动处理
     - **方式 2**：使用明确指令 `/confirm-fix` 或 `/approve`（快捷通道）
   - ❌ **拒绝**：在评论中说明调整要求
